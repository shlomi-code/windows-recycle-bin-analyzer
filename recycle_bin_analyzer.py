...
try:
    ...
    print("Warning: win32api not available. Install pywin32 for better performance.")
...
class RecycleBinAnalyzer:
    ...
    def _get_current_user_sid(self) -> Optional[str]:
        ...
        if not WINDOWS_API_AVAILABLE:
            return self._get_current_user_sid_fallback()
    ...
    def _get_current_user_sid_fallback(self) -> Optional[str]:
    ...
    def _get_all_user_sids(self) -> List[str]:
        ...
        if not WINDOWS_API_AVAILABLE:
            return self._get_all_user_sids_fallback()
    ...
    def _get_all_user_sids_fallback(self) -> List[str]:
        """Fallback method using subprocess if Windows API is not available."""
        import subprocess
        sids = []
        try:
